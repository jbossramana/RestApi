...
public class RestApiController {
    ...
    public ResponseEntity<List<User>> listAllUsersPagination(
        @Parameter(description = "Page number (default is 0)", example = "0") @RequestParam(defaultValue = "0") Integer pageNo,
        @Parameter(description = "Page size (default is 10)", example = "10") @RequestParam(defaultValue = "10") Integer pageSize,
        @Parameter(description = "Sort by: name, age, or salary", example = "name") @RequestParam(defaultValue = "id") String sortBy,
        @Parameter(description = "Minimum salary filter", example = "50000") @RequestParam(required = false) Double minSalary
    ) {
        List<User> users = userService.findAllUsers();
        if (minSalary != null) {
    ...
    public ResponseEntity<?> getUser(
        @Parameter(description = "ID of the user to retrieve", example = "101") @PathVariable("id") long id) {
    ...
    public ResponseEntity<?> createUser(
        @RequestBody(description = "User object to be created", required = true, content = @Content(schema = @Schema(implementation = User.class))) @org.springframework.web.bind.annotation.RequestBody User user,
    ...
    public ResponseEntity<?> updateUser(
        @Parameter(description = "ID of the user to update", example = "101") @PathVariable("id") long id,
        @RequestBody(description = "Updated user object", required = true, content = @Content(schema = @Schema(implementation = User.class))) @org.springframework.web.bind.annotation.RequestBody User user) {
    ...
    public ResponseEntity<?> deleteUser(
        @Parameter(description = "ID of the user to delete", example = "101") @PathVariable("id") long id) {
